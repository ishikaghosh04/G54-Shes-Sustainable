  ...
  useEffect(() => {
    orders.forEach((order, idx) => {
      API.get(`/shipping/order/${order.orderID}`)
         .then(res => {
           const shipments = res.data; // [{ shippingID, productName, trackingNumber, estDeliveryDate, sellerFirstName… }]
           // for each shipment, fetch review if any
           Promise.all(
             shipments.map(s =>
              ...
              return (
                <div key={sh.shippingID} className="shipment-record">
                  <p>
                    ...
                    <strong>ETA:</strong> {new Date(sh.estDeliveryDate).toLocaleDateString()}
                  </p>
                  <p>
                    <strong>Seller:</strong> {sh.sellerFirstName} {sh.sellerLastName}<br/>
                    <a href={`mailto:${sh.sellerEmail}`}>{sh.sellerEmail}</a>
                  </p>
                  ...
                  {sh.review ? (
                    <div className="existing-review">
                      <div className="star-rating read-only">
                        {[1,2,3,4,5].map(i => (
                          <span key={i} className={i <= sh.review.rating ? 'star active' : 'star'}>★</span>
                        ))}
                      </div>
                      <p>{sh.review.comment}</p>
                    </div>
                  ) : (
                    <button
                      ...
                      onClick={() => navigate('/review', {
                        state: {
                          productID: order.items.find(it => it.productID === sh.productID).productID,
